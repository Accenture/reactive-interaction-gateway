defmodule RigInboundGatewayWeb.V1.MetadataController do
  ...
  use Rig.Config, [:cors, :jwt_fields, :indexed_metadata]
  ...
  ## Dirty testing with JWT
      ...
      http put ":4000/_rig/v1/connection/${CONN_TOKEN}/metadata" <<< "$META" "$AUTH"

  ## Dirty testing without JWT
      META='{ "metadata": { "userid": "9ab1bff2-a8d8-455c-b48a-50145d7d8e30", "locale": "de-AT", "timezone": "GMT+2" } }'
      http put ":4000/_rig/v1/connection/${CONN_TOKEN}/metadata" <<< "$META"
  ...
  defp persist_metadata(metadata, connection_id) do
    ...
    # TODO: Actually set metadata, fix the hack below (this should be in a with statement)
    {:ok, socket_pid} = Connection.Codec.deserialize(connection_id)
    send(socket_pid, {:set_metadata, metadata})
  end
