defmodule RIG.DistributedMap do
  @moduledoc """
  ...
  @type record :: {key(), value(), uuid(), creation_ts(), expiration_ts()}
  ...
  @doc """
  Creates a new distributed map.

  ## Parameters

    - map_name: The name of the distributed map. Multiple instances with the same name
    will synchronize their records with each other.

  """
  def start_link(map_name, opts \\ []) when is_atom(map_name) do
    pg2_group = opts[:pg2_group] || pg2_group_name(map_name)
  ...
  def has?(pid, key, opts) do
    ...
    query =
      fun do
        {^key, _, _, _, exp} when exp > ^now -> true
      end
  ...
  @doc """
  Get a (or multiple) value by its key.

  Returns an array of values by key.
  """
  @spec get(pid | atom, key, list) :: list(value)
  def get(name, key, opts \\ [])

  def get(name, key, opts) when is_atom(name) do
    pid = Process.whereis(name)
    get(pid, key, opts)
  end

  def get(pid, key, opts) do
    if has?(pid, key, opts) do
      ets_table = ets_table_name(pid)
  ...
  def init(%{name: name, opts: opts, pg2_group: pg2_group} = state) do
    ...
    Logger.info(fn ->
      ...
      "Initialized distributed map #{name} (#{pg2}, #{ets})"
    end)
  ...
  def handle_call(
    ...
    query =
      case since_record_id do
        nil ->
          fun do
            {_, _, _, from_ts, _} -> from_ts
          end

        _ ->
          fun do
            {_, _, id, from_ts, _} when id == ^since_record_id -> from_ts
          end
      end
  ...
  @impl GenServer
  def handle_call(
        {:add_from_local, {_, _, uuid, _, _} = record},
        _from,
  ...
  @impl GenServer
  def handle_info(
        {:add_from_remote, {_, _, incoming_record_id, _, _} = record, remote_previous_record_id},
      ...
      case local_last_record_id do
        ...
        _ ->
          ...
          Logger.info(fn ->
            ...
            "Metadata nodes out of sync (#{local} vs #{remote})"
          end)
  ...
  defp get_records(ets_table, from_ts) do
    query =
      fun do
        {_, _, _, creation_ts, _} = record when creation_ts >= ^from_ts -> record
      end
  ...
  defp pg2_group_name(name) when is_atom(name), do: {:distributed_map, name}

  defp ets_table_name(pid),
    do: "distributed_map_#{:erlang.pid_to_list(pid)}" |> String.to_atom()
