defmodule Gateway.Kafka.SupWrapper do
  ...
  `Gateway.Kafka.Sup`. In case no Kafka broker is available at startup,
  however, `Gateway.Kafka.Sup` fails to start, bringing down the application.
  This might not be desirable, as the Gateway might be started before Kafka
  comes up. In case a disaster happens, this makes booting up the overall
  system easier.

  We trap only `:failed_to_start_child` here; all other errors will crash
  this server, and thus propagate up.
