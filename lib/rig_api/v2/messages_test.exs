defmodule RigApi.V2.MessagesTest do
  @moduledoc false
  ...
  setup do
    ...
    Rig.EventFilterMock
    |> stub(:forward_event, fn ev, _eventhub, _topic -> send(self(), {:cloud_event_sent, ev}) end)
  ...
  describe "Structured mode" do
    test "is supported for content-type application/cloudevents+json." do
      conn =
        ...
        |> post("/v2/messages", @cloud_event_json)
    ...
    test "rejects events that don't follow the CloudEvents format." do
      conn =
        new_conn("application/cloudevents+json;charset=utf-8")
        |> post("/v2/messages", @non_cloud_event)
