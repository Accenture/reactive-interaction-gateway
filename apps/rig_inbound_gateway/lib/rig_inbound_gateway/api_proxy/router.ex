defmodule RigInboundGateway.ApiProxy.Router do
  ...
  @spec pick_api_and_endpoint([Proxy.api_definition()], String.t(), String.t()) ::
          {Proxy.api_definition(), Proxy.endpoint()}
  ...
  @spec check_auth_and_forward_request(Proxy.endpoint(), Proxy.api_definition(), Plug.Conn.t()) ::
          Plug.Conn.t()
  ...
  @spec check_auth_and_forward_request(Proxy.endpoint(), Proxy.api_definition(), Plug.Conn.t()) ::
          Plug.Conn.t()
  defp check_auth_and_forward_request(
  ...
  defp check_request_type(%{"type" => "sync", "target" => "kinesis"}, _api, conn) do
    ...
    send_response(
      {:ok, conn,
       %{body: response_json, status_code: 501, headers: [{"content-type", "application/json"}]}}
    )
  end
